...
class Template {
    ...
    public $_ci;
    protected $brand_name = 'FATH Framework';
    ...
    function __construct() {
        ...
        if ($this->_ci->session->userdata('_fath__platform_') === 'app') {
    ...
    public function load_view($view, $data = array(), $return = FALSE) {
        if ($this->_ci->input->is_ajax_request()) {
            ...
            foreach ($this->js as $js_file) {
                $js[] = '<script src="' . base_url() . 'fath-assets/javascripts/' . $js_file . '" type="text/javascript"></script>';
            }
            $js = implode('', $js);

            $css = array();
            foreach ($this->css as $css_file) {
                $css[] = '<link type="text/css" rel="stylesheet" href="' . base_url() . 'fath-assets/stylesheets/' . $css_file . '"/>';
            }
            $css = implode('', $css);
